...
函数极限的定义按自变量的变化过程不同有各种形式，设 $\lim f(x) = A, \forall \varepsilon \gt 0$，则：

| $x\rightarrow$ | 存在 | 当 | 有 |
|----------------|------|---|----|
| $+\infty$ | $\exists X \gt 0$ | $x \gt X$ | $\vert f(x)-A \vert \lt \varepsilon$ |
| $-\infty$ | $\exists X \gt 0$ | $x \lt -X$ | $\vert f(x)-A \vert \lt \varepsilon$ |
| $\infty$ | $\exists X \gt 0$ | $\vert x \vert \gt X$ | $\vert f(x)-A \vert \lt \varepsilon$ |
| $x_0^+$ | $\exists \delta \gt 0$ | $x_0 \lt x \lt x_0 + \delta$ | $\vert f(x)-A \vert \lt \varepsilon$ |
| $x_0^-$ | $\exists \delta \gt 0$ | $x_0-\delta \lt x \lt x_0$ | $\vert f(x)-A \vert \lt \varepsilon$ |
| $x_0$ | $\exists \delta \gt 0$ | $0 \lt \|x - x_0\| \lt \delta$ | $\vert f(x)-A \vert \lt \varepsilon$ |
...
1. $x\rightarrow x_0$
    ...
    **证明**
    ...
    因为 $\lim\limits_{x\rightarrow x_0}f(x) = A$，所以取 $\varepsilon=1$，则 $\exists \delta \gt 0$，当 $0\lt |x-x_0| \lt \delta$ 时，有 
...
3. $x\rightarrow x_0^-$
    ...
    如果 $\lim\limits_{x\rightarrow x_0^-}f(x) = A$，那么存在常数 $M\gt 0$ 和 $\delta \gt 0$，使得当 $0 \gt x - x_0 \gt -\delta$时，有 $|f(x)| \leqslant M$

    **证明**

    因为 $\lim\limits_{x\rightarrow x_0^-}f(x) = A$，所以取 $\varepsilon=1$，则 $\exists \delta \gt 0$，当 $0\gt x-x_0 \gt -\delta$ 时，有 
...
4. $x\rightarrow \infty$
    ...
    如果 $\lim\limits_{x\rightarrow \infty}f(x) = A$，那么存在常数 $M\gt 0$ 和 $X \gt 0$，使得当 $|x| \gt X$时，有 $|f(x)| \leqslant M$

    **证明**

    因为 $\lim\limits_{x\rightarrow \infty}f(x) = A$，所以取 $\varepsilon=1$，则 $\exists X\gt 0$，当 $|x|\gt X$ 时，有 
