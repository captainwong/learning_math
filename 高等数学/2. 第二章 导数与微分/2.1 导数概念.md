- [第一节 导数概念](#第一节-导数概念)
  - [一、引例](#一引例)
    - [1. 瞬时速度](#1-瞬时速度)
    - [2. 切线斜率](#2-切线斜率)
  - [二、导数的定义](#二导数的定义)
    - [1. 函数在一点处的导数与导函数](#1-函数在一点处的导数与导函数)
      - [1.1 定义](#11-定义)
      - [1.2 定义式的其他形式](#12-定义式的其他形式)
      - [1.3 区间可导](#13-区间可导)
      - [1.4 导函数](#14-导函数)
    - [2. 求导数举例](#2-求导数举例)
      - [2.1 $f(x) = C$ ( $C$ 为常数) 的导数](#21-fx--c--c-为常数-的导数)
      - [2.2 $f(x) = x^n$ ( $n\in \N_+$) 的导数](#22-fx--xn--nin-n_-的导数)
      - [2.3 $f(x)=x^\mu(\mu \in \R)$ 的导数](#23-fxxmumu-in-r-的导数)
      - [2.4 $f(x)=\sin x$ 的导数](#24-fxsin-x-的导数)
      - [2.5 $f(x)=\cos x$ 的导数](#25-fxcos-x-的导数)
      - [2.6 $f(x) = a^x \kern8px (a\gt 0, a\ne 1)$ 的导数](#26-fx--ax-kern8px-agt-0-ane-1-的导数)
      - [2.7 $f(x) = \log_a x \kern8px (a\gt 0, a\ne 1)$ 的导数](#27-fx--log_a-x-kern8px-agt-0-ane-1-的导数)
    - [3. 单侧导数](#3-单侧导数)
  - [三、导数的几何意义](#三导数的几何意义)
  - [四、函数可导性与连续性的关系](#四函数可导性与连续性的关系)
  - [习题2-1](#习题2-1)
    - [1. 设物体绕定轴旋转，在时间间隔 $[0,t]$ 上转过角度 $\theta$，从而转角 $\theta$ 是 $t$ 的函数： $\theta = \theta(t)$. 如果旋转是匀速的，那么称 $\omega = \frac \theta t$ 为该物体旋转的**角速度**. 如果旋转是非匀速的，应怎样确定该物体在时刻 $t_0$ 的角速度?](#1-设物体绕定轴旋转在时间间隔-0t-上转过角度-theta从而转角-theta-是-t-的函数-theta--thetat-如果旋转是匀速的那么称-omega--frac-theta-t-为该物体旋转的角速度-如果旋转是非匀速的应怎样确定该物体在时刻-t_0-的角速度)
    - [2. 物体冷却速度](#2-物体冷却速度)
    - [3. 成本函数与边际成本](#3-成本函数与边际成本)
    - [4. 按定义求显函数的导数值，就是按定义来做，别用导数公式.](#4-按定义求显函数的导数值就是按定义来做别用导数公式)
    - [6. 下列各题中均假定 $f'(x)$ 存在，按导数定义观察下列极限，指出 $A$ 代表什么](#6-下列各题中均假定-fx-存在按导数定义观察下列极限指出-a-代表什么)
    - [7. 设](#7-设)
    - [8. 设 $f(x)$ 可导， $F(x)=f(x)(1+|\sin x|)$, 则 $f(0)=0$ 是 $F(x)$ 在 $x=0$ 处可导的（ ）.](#8-设-fx-可导-fxfx1sin-x-则-f00-是-fx-在-x0-处可导的-)
    - [11. 如果 $f(x)$ 为偶函数，且 $f'(0)$ 存在，证明 $f'(0)=0$](#11-如果-fx-为偶函数且-f0-存在证明-f00)
    - [19. 已知](#19-已知)

# 第一节 导数概念

## 一、引例

### 1. 瞬时速度

$v=\lim\limits_{t\rightarrow t_0}\frac{f(t) - f(t_0)}{t-t_0}$

### 2. 切线斜率

$k = \lim\limits_{x\rightarrow x_0}\frac{f(x) - f(x_0)}{x-x_0}$

## 二、导数的定义

### 1. 函数在一点处的导数与导函数

#### 1.1 定义

设函数 $y=f(x)$ 在点 $x_0$ 的某个邻域内有定义，当自变量 $x$ 在 $x_0$ 处取得增量 $\Delta x$ (点 $x_0+\Delta x$ 仍在该邻域内)时，相应地，因变量取得增量 $\Delta y = f(x_0 + \Delta x) - f(x_0)$; 如果 $\Delta y$ 与 $\Delta x$ 之比当 $\Delta x \rightarrow 0$ 时的极限存在，那么称函数 $y=f(x)$ 在点 $x_0$ 处**可导**，并称这个极限为函数 $y=f(x)$ 在点 $x_0$ 处的**导数**，记为 $f'(x)$, 即

$$
f'(x_0) = \lim\limits_{\Delta x \rightarrow 0}\frac{\Delta y}{\Delta x} = \lim\limits_{\Delta x \rightarrow 0}\frac{f(x_0+\Delta x) - f(x_0)}{\Delta x}
$$

也可以记作 

$$
y'|_{x=x_0},
$$

$$
\frac{dy}{dx}|_{x=x_0},
$$

$$
\frac{df(x)}{dx}|_{x=x_0}
$$

#### 1.2 定义式的其他形式

- $f'(x_0) = \lim\limits_{h\rightarrow 0}\frac{f(x_0+h) - f(x_0)}{h}$
- $f'(x_0) = \lim\limits_{x\rightarrow x_0}\frac{f(x) - f(x_0)}{x-x_0}$

#### 1.3 区间可导

如果函数 $y=f(x)$ 在开区间 $I$ 内的每点处都可导，那么称函数 $f(x)$ 在开区间 $I$ 可导.

#### 1.4 导函数

函数 $f(x)$ 在开区间 $I$ 可导，对于任一 $x\in I$，都对应 $f(x)$ 的一个确定的导数值. 这样就构成了一个新的函数，称为原来函数 $y=f(x)$ 的导函数，记作 $y', f'(x), \frac{dy}{dx}$ 或 $\frac{df(x)}{dx}$.

导函数的定义式：

$y' = \lim\limits_{\Delta x \rightarrow 0}\frac{f(x+\Delta x) - f(x)}{\Delta x}$

或

$f'(x) = \lim\limits_{h\rightarrow 0}\frac{f(x+h) - f(x)}{h}$

在极限过程中， $x$ 是常量， $\Delta x$ 或 $h$ 是变量.

### 2. 求导数举例

#### 2.1 $f(x) = C$ ( $C$ 为常数) 的导数

$f'(x) = \lim\limits_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h} = \lim\limits_{h\rightarrow 0}\frac{C-C}{h} = 0$

#### 2.2 $f(x) = x^n$ ( $n\in \N_+$) 的导数

当 $n=1$ 时， $f'(x) = \lim\limits_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h} = \lim\limits_{h\rightarrow 0}\frac{(x+h)-x}{h} = 1$.

当 $n\gt 1$ 时，由牛顿二项公式

$$
(a+b)^n = \displaystyle\sum_{k=0}^n C_n^k a^{n-k} b^k
$$

$$
\begin{align}
f'(x) & = \lim\limits_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac{(x+h)^n - x^n}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac 1 h[nx^{n-1}h + \frac{n(n-1)}2 x^{n-2}h^2 + \cdots + h^n] \\
& = \lim\limits_{h\rightarrow 0}(nx^{n-1}+ \frac{n(n-1)}2 x^{n-2}h + \cdots + h^{n-1}) \\
& = nx^{n-1}
\end{align}
$$

即

$$
(x^n)' = 
\begin{cases}
1 &, n=1, \\
nx^{n-1} &, n\gt 1.
\end{cases}
$$

#### 2.3 $f(x)=x^\mu(\mu \in \R)$ 的导数

$$
\begin{align}
f'(x) & = \lim\limits_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac{(x+h)^\mu - x^\mu}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac{x^\mu}h [(1+\frac h x)^\mu - 1] \\
& = \lim\limits_{h\rightarrow 0}\frac{x^\mu}h \cdot \mu \cdot \frac h x \\
& = \mu x^{\mu - 1}
\end{align}
$$

即

$$
(x^\mu)' = \mu x^{\mu - 1}
$$

#### 2.4 $f(x)=\sin x$ 的导数

$$
\begin{align}
f'(x) &= \lim\limits_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac{\sin(x+h) - \sin x}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac 1 h \cdot 2\cos(x+\frac h 2)\sin\frac h 2 \\
& = \lim\limits_{h\rightarrow 0}\cos(x+\frac h 2) \cdot \frac{\sin \frac h 2}{\frac h 2} \\
& = \cos x
\end{align}
$$

#### 2.5 $f(x)=\cos x$ 的导数

$$
\begin{align}
f'(x) &= \lim\limits_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac{\cos(x+h) - \cos x}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac 1 h \cdot (-2)\sin(x+\frac h 2)\sin\frac h 2 \\
& = \lim\limits_{h\rightarrow 0}-\sin(x+\frac h 2) \cdot \frac{\sin \frac h 2}{\frac h 2} \\
& = -\sin x
\end{align}
$$

#### 2.6 $f(x) = a^x \kern8px (a\gt 0, a\ne 1)$ 的导数

$$
\begin{align}
f'(x) &= \lim\limits_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac{a^{x+h} - a^x}{h} \\
& = a^x \cdot \lim\limits_{h\rightarrow 0}[\frac 1 h \cdot (a^h - 1)] \\
& = a^x \cdot \lim\limits_{h\rightarrow 0}\frac 1 h \cdot h \cdot \ln a \\
& = a^x \ln a
\end{align}
$$

即 $(a^x)' = a^x \ln a$

当 $a=e$ 时， $(e^x)' = e^x \ln e = e^x$

#### 2.7 $f(x) = \log_a x \kern8px (a\gt 0, a\ne 1)$ 的导数

$$
\begin{align}
f'(x) &= \lim\limits_{h\rightarrow 0}\frac{f(x+h)-f(x)}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac{\log_a(x+h) - \log_ax}{h} \\
& = \lim\limits_{h\rightarrow 0}\frac 1 h \cdot \log_a(1+\frac h x) \\
& = \lim\limits_{h\rightarrow 0}\frac 1 h \cdot \frac{\ln(1+\frac h x)}{\ln a} \\
& = \lim\limits_{h\rightarrow 0}\frac 1 h \cdot \frac{\frac h x}{\ln a} \\
& = \frac 1 {x\ln a}
\end{align}
$$

即 $(\log_ax)'=\frac 1{x\ln a}$

当 $a=e$， $(\ln x)' = \frac 1 x$

### 3. 单侧导数

根据导数定义，导数是极限，而极限存在的充分必要条件是左右极限都存在且相等，因此 $f(x)$ 在点 $x_0$ 处可导的充分必要条件是**左导数** 和**右导数** 都存在且相等.

$f^{\prime}_{-}(x_0)$

$= \lim\limits_{h\rightarrow 0^-}\frac{f(x_0+h)-f(x_0)}{h}$

$f^{\prime}_{+}(x_0)$

$= \lim\limits_{h\rightarrow 0^+}\frac{f(x_0+h)-f(x_0)}{h}$

$f(x)$ 在 $[a,b]$ 可导 $\iff$ $f(x)$ 在 $(a,b)$ 可导，且 

$f^{\prime}_{+}(a)$,

$f^{\prime}_{-}(b)$

存在.

*MathJax对单侧导数的支持不好*

## 三、导数的几何意义

曲线 $y=f(x)$ 在点 $M(x_0,y_0)$ 处的**切线方程**为 $y-y_0=f'(x_0)(x-x_0)$

曲线 $y=f(x)$ 在点 $M(x_0,y_0)$ 处的**法线方程**为 $y-y_0=-\frac 1 {f'(x_0)}(x-x_0)$

## 四、函数可导性与连续性的关系

函数 $y=f(x)$ 在点 $x$ 处可导 $\iff \lim\limits_{\Delta x \rightarrow 0}\frac{\Delta y}{\Delta x} = f'(x)$

$\implies \frac{\Delta y}{\Delta x} = f'(x) + \alpha$，其中 $\alpha$ 为当 $\Delta x \rightarrow 0$ 时的无穷小.

$\implies \Delta y = \Delta xf'(x) + \alpha \Delta x$

$\implies$ 当 $\Delta x \rightarrow 0, \Delta y \rightarrow 0$

$\implies y=f(x)$ 在点 $x$ 连续.

综上，**可导必连续，连续未必可导**.

## 习题2-1

###  1. 设物体绕定轴旋转，在时间间隔 $[0,t]$ 上转过角度 $\theta$，从而转角 $\theta$ 是 $t$ 的函数： $\theta = \theta(t)$. 如果旋转是匀速的，那么称 $\omega = \frac \theta t$ 为该物体旋转的**角速度**. 如果旋转是非匀速的，应怎样确定该物体在时刻 $t_0$ 的角速度?

**解** 严格按照定义来做，并且要用题目中给出的函数表示结果

物体在时间间隔 $[t_0, t_0+\Delta t]$ 上的平均角速度为

$\overline{\omega} = \frac{\Delta \theta}{\Delta t}=\frac{\theta(t_0+\Delta t) - \theta(t_0)}{\Delta t}$ 

在时刻 $t_0$ 的角速度为

$\omega = \lim\limits_{\Delta t \rightarrow 0}\overline{\omega} = \lim\limits_{\Delta t \rightarrow 0}\frac{\Delta \theta}{\Delta t} = \theta'(t_0)$


### 2. 物体冷却速度

同1，关键还是导数定义的几个形式要牢固记忆，并在结果中用题目中的函数表示出来

### 3. 成本函数与边际成本

（2）边际成本 $C'(x)$ 的实际意义是近似表达产量达到 $x$ 单位时，再增加一个单位产品所需的成本.

### 4. 按定义求显函数的导数值，就是按定义来做，别用导数公式.

### 6. 下列各题中均假定 $f'(x)$ 存在，按导数定义观察下列极限，指出 $A$ 代表什么

(1) $\lim\limits_{\Delta x\rightarrow 0}\frac{f(x_0-\Delta x) - f(x_0)}{\Delta x} = A$

(2) $\lim\limits_{x\rightarrow 0}\frac{f(x)}{x} = A$, 其中 $f(0)=0$，且 $f'(0)$ 存在.

(3) $\lim\limits_{h\rightarrow 0}\frac{f(x_0+h) - f(x_0-h)}{h} = A$

**解**

(1) $\lim\limits_{\Delta x\rightarrow 0}\frac{f(x_0-\Delta x) - f(x_0)}{\Delta x} = -\lim\limits_{\Delta x\rightarrow 0}\frac{f(x_0-\Delta x) - f(x_0)}{-\Delta x} = -f'(x_0)$

(2) $\lim\limits_{x\rightarrow 0}\frac{f(x)}{x}=\lim\limits_{x\rightarrow 0}\frac{f(x)-f(0)}{x-0}=f'(0)$

(3) $\lim\limits_{h\rightarrow 0}\frac{f(x_0+h) - f(x_0-h)}{\Delta x}=\lim\limits_{h\rightarrow 0}\frac{f(x_0+h) -f(x_0) + f(x_0) - f(x_0-h)}{h}=\lim\limits_{h\rightarrow 0}\frac{f(x_0+h) - f(x_0)}{h} + \lim\limits_{h\rightarrow 0}\frac{f(x_0-h) - f(x_0)}{-h}=2f'(x_0)$

**关键还是在凑导数定义形式**

### 7. 设

$$
f(x)=
\begin{cases}
\frac 2 3 x^3 & , x\leqslant 1, \\
x^2 & , x \gt 1.
\end{cases}
$$

则 $f(x)$ 在 $x=1$ 处的（ ）.

A. 左右导数都存在

B. 左导数存在，右导数不存在

C. 左导数不存在，右导数存在

D. 左右导数都不存在

**解**

$f^{\prime}_{-}(1)$

$$
\begin{align}
& =\lim\limits_{x\rightarrow 1^-}\frac{f(x)-f(1)}{x-1} \\
& = \lim\limits_{x\rightarrow 1^-}\frac{\frac 2 3 x^3 - \frac 2 3}{x-1} \\
& = \lim\limits_{x\rightarrow 1^-}\frac 2 3 \cdot \frac{(x-1)(x^2+x+1)}{x-1} \\
& = \lim\limits_{x\rightarrow 1^-}\frac 2 3 \cdot (x^2 + x + 1) \\
& = 2
\end{align}
$$

$f^{\prime}_{+}(1)$

$$
\begin{align}
& =\lim\limits_{x\rightarrow 1^+}\frac{f(x)-f(1)}{x-1} \\
& = \lim\limits_{x\rightarrow 1^+}\frac{x^2 - \frac 2 3}{x-1} \\
& = \infty
\end{align}
$$

关键错处在 $f(1)$ 的计算，这是个定值，直接套 $f(x)$ 的公式即可，不要考虑 $x$ 的极限过程， $x$ 的极限过程跟你周树人有什么关系？？？

### 8. 设 $f(x)$ 可导， $F(x)=f(x)(1+|\sin x|)$, 则 $f(0)=0$ 是 $F(x)$ 在 $x=0$ 处可导的（ ）.

A. 充分必要条件

B. 充分非必要条件

C. 必要非充分条件

D. 既非充分又非必要条件

**解** 没思路就写出 $F(x)$ 在 $x=0$ 处可导的充要条件（左导数=右导数），看能不能得出与 $f(0)=0$ 的关系.

$F^{\prime}_{-}(0)$

$$
\begin{align}
& = \lim\limits_{x\rightarrow 0^-}\frac{F(x)-F(0)}{x-0} \\
& = \lim\limits_{x\rightarrow 0^-}\frac{f(x)(1-\sin x)-f(0)}{x} \\
& = \lim\limits_{x\rightarrow 0^-}\frac{f(x) - f(x)\sin x - f(0)}{x} \\
& = \lim\limits_{x\rightarrow 0^-}\frac{f(x)-f(0)}{x} - \lim\limits_{x\rightarrow 0^-}f(x) \cdot \frac {\sin x} x \\
& = f'(0) - \lim\limits_{x\rightarrow 0^-}f(x) \\
& = f'(0) - f(0)
\end{align}
$$

$F^{\prime}_{+}(0)$

$$
\begin{align}
& = \lim\limits_{x\rightarrow 0^+}\frac{F(x)-F(0)}{x-0} \\
& = \lim\limits_{x\rightarrow 0^+}\frac{f(x)(1+\sin x) - f(0)}{x} \\
& = \lim\limits_{x\rightarrow 0^+}\frac{f(x) + f(x)\sin x - f(0)}{x} \\
& = \lim\limits_{x\rightarrow 0^+}\frac{f(x)-f(0)}{x} + \lim\limits_{x\rightarrow 0^+}f(x) \cdot \frac {\sin x} x \\
& = f'(0) + \lim\limits_{x\rightarrow 0^+}f(x) \\
& = f'(0) + f(0)
\end{align}
$$

$f(0) = 0 \iff$ 

$F^{\prime}_{-}(0)$ 

$=$ 

$F^{\prime}_{+}(0)$

### 11. 如果 $f(x)$ 为偶函数，且 $f'(0)$ 存在，证明 $f'(0)=0$

**证** 思路： $f'(0)$ 存在 $\implies f(x)$ 在 $x=0$ 连续，不必要用左导右导分别求了，直接求导.

$f(x)$ 为偶函数 $\implies f(-x)=f(x)$

$$
\begin{align}
f'(0) & = \lim\limits_{x\rightarrow 0}\frac{f(x)-f(0)}{x-0} \\
& = \lim\limits_{x\rightarrow 0}\frac{f(-x)-f(0)}{x-0} \\
& = -\lim\limits_{-x\rightarrow 0}\frac{f(-x)-f(0)}{-x-0} \\
& = -f'(0) \\
\implies 2f'(0) & = 0 \\
\therefore f'(0) & = 0
\end{align}
$$

### 19. 已知

$$
f(x)=
\begin{cases}
\sin x & , x\lt 0, \\
x & , x \geqslant 0,
\end{cases}
$$

求 $f'(x)$.

**解** 关键在判断分段处的导数，否则口算都完成了.


